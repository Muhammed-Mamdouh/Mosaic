...
from mosiac import app, widths,heights,tile_sizes, closest_paths_list,main_photo_sizes,output_names,main_photo_paths
...
def home_page():
    main_photos = [r"../"+'/'.join(path.replace(r'\\','/').split('/')[1:]).replace("'", "") for path in main_photo_paths]
    return render_template('home.html', main_photos=main_photos)

@app.route('/grid/<n>')
def grid_page(n):
    n = int(n)
    output_name = r"../"+'/'.join(output_names[n].replace(r'\\','/').split('/')[1:]).replace("'","")
    print(output_name)
    w = widths[n]
    max_width = str(100/w)+"%"
    ...
    for i in range(closest_paths_list[n].shape[0]):
        ...
        for j in range(closest_paths_list[n].shape[1]):
            closest_objects[i,j] = (r"../"+'/'.join(closest_paths_list[n][i,j].replace(r'\\','/').split('/')[1:]).replace("'",""),n,l,m)
            m += 1
        l += 1
...
def image_page(n, l, m):
    ...
    path = r"../../"+'/'.join(raw.replace(r'\\','/').split('/')[1:]).replace("'","").replace("oj_tiles","tiles")
